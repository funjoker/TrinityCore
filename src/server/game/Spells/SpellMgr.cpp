...
void SpellMgr::LoadSpellInfoCorrections()
{
    ...
    // Some spells have no amplitude set
    {
        ...
        {
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Amplitude = 1 * IN_MILLISECONDS;
        });
        ...
        }, [](SpellInfo* spellInfo)
        {
            ...
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->Amplitude = const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_0))->Amplitude;
        });

        // Vomit
        ApplySpellFix({ 43327 }, [](SpellInfo* spellInfo)
        {
            const_cast<SpellEffectInfo*>(spellInfo->GetEffect(EFFECT_1))->Amplitude = 1 * IN_MILLISECONDS;
        });
    }
